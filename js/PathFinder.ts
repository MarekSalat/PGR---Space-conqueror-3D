...
class PathFinder {
    ...
    findPath(from, to)
    {
        ...
        while (from.position != to.position && radDist < dist && j < 100)
        {
            ...
            if (i < intersects.length && intersects[i].object.position != to.position &&
                distance > intersects[i].distance)
            {
                ...
                if ((startVec.length() > endVec.length() && alpha < lineAlpha) ||
                    (startVec.length() < endVec.length() && alpha > lineAlpha))
                {
                    obstruction = endVec;
                    alpha = /*Math.abs*/(Math.atan(obstruction.y / obstruction.x));
                    alpha = Math.abs(alpha);
                }
